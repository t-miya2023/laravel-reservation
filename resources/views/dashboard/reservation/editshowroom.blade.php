...
<!--1部屋の場合------------------------------------------------------------------------------------------>
            @if($roomNumber  == 1)
                    ...
                    <div class="row bg-white p-3 mb-3 mw-800 border border-warning border-3 position-relative">
                        ...
                        <form action="{{ route('dashboard.reservation.editconfirm',$reservation->id) }}" method="post">
                            @csrf
...
<!--2部屋の場合------------------------------------------------------------------------------------------>
        @else
            <div id="error-message" style="color: red;"></div>
                <div id="error-message" style="color: red;"></div>
                <form  id="roomField"action="{{ route('dashboard.reservation.editconfirm',$reservation->id) }}" method="post">
                    <!--予約中の部屋-->
                    @foreach ($reservation->reservation_details as $key => $detail)
                    <div class="row bg-white p-3 mb-3 mw-800 room border border-warning border-3 position-relative" data-capacity="{{ $detail->room->capacity }}">
                        ...
                        <div class="d-flex justify-content-between border-bottom mb-2">
                            <h3>{{ $detail->room->name }}</h3>  
                            <a class="btn waves-effect waves-light" href="{{ route('room.show',$detail->room->id)  }}">詳細確認</a>
                        </div>
                        <div class="col-md-3 col-12">
                            <div class="room-img-box">
                            @if( $detail->room->img)
                        ...
                        <div class="col-7">
                            <select class="form-select" name="number_of_guests[]" aria-label="Default select example">
                    ...
                    <div class="row bg-white p-3 mb-3 mw-800 room" data-capacity="{{ $room->capacity }}">
                        <div class="d-flex justify-content-between border-bottom mb-2">
                            <h3>{{ $room->name }}</h3>  
                            <a class="btn waves-effect waves-light" href="{{ route('room.show',$room->id)  }}">詳細確認</a>
                        </div>
                        <div class="col-md-3 col-12">
                            <div class="room-img-box">
                            @if($room->img)
